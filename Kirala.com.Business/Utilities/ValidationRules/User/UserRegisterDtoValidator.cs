    ...
    public class UserRegisterDtoValidator : AbstractValidator<UserRegisterDto>
    {
        public UserRegisterDtoValidator()
        {
            ...
            RuleFor(x => x.Phone).NotNull().NotEmpty().WithMessage(ConstantMessage.UserPhoneNotNull)
                .MinimumLength(11).WithMessage(ConstantMessage.UserPhoneMinChar)
                .MaximumLength(14).WithMessage(ConstantMessage.UserPhoneMaxChar)
                .Matches(new Regex("(([+][(]?[0-9]{1,3}[)]?)|([(]?[0-9]{4}[)]?))\\s*[)]?[-\\s\\.]?[(]?[0-9]{1,3}[)]?([-\\s\\.]?[0-9]{3})([-\\s\\.]?[0-9]{3,4})"))
                .WithMessage(ConstantMessage.UserPhoneFormat);
